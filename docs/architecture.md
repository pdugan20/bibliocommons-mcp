...
| Quirk                                             | Notes                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Digital vs. physical have separate endpoints.** | Available digital items → `POST /v2/libraries/{library}/checkouts` (immediate borrow). Physical → `POST /holds`. Unavailable digital (Libby waitlist) → `POST /holds` with `materialType: "DIGITAL"`, but this needs a `format` enum field we haven't fully exercised yet (deferred to v1.1; the Libby app handles digital waitlists fine). |
