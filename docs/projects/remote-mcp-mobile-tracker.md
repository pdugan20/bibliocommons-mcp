...
| 0.5 | Decide **hosting**: Cloud Run vs Fly vs Railway, and create the account/project. Must be a **warm** host (Cloud Run `min-instances=1` / Fly always-on) so the per-session cookie cache survives — see 0.2                                                                                                                            | owner      | [ ]    |
